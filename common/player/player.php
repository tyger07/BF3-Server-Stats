...
elseif($SoldierName != null)
{
	...
	if(!empty($PlayerID))
	{
		...
		if(!empty($ServerID))
		{
			...
			if($adkats_available)
			{
				$PlayerData_q = @mysqli_query($BF3stats,"
	...
	if($soldier_found == 0)
	{
		...
		if(!empty($ServerID))
		{
			...
			if($adkats_available)
			{
				$PlayerMatch_q = @mysqli_query($BF3stats,"
	...
	elseif($soldier_found == 1)
	{
		...
		if(@mysqli_num_rows($Weapon_q) != 0 || $VehicleFound == 1)
		{
			...
			Statsout($WeaponCodes['0'], $weapon_array, $PlayerID, $ServerID, $valid_ids, $GameID, $BF3stats, '3');
			echo '</div>
			</div>
			<br/>
			';
		...
		if(!empty($ServerID))
		{
			$DogTag_q1 = @mysqli_query($BF3stats,"
				SELECT tpd.`PlayerID`
				FROM `tbl_dogtags` dt
		...
		if(!empty($ServerID))
		{
			$DogTag_q2 = @mysqli_query($BF3stats,"
				SELECT tpd.`PlayerID`
				FROM `tbl_dogtags` dt
