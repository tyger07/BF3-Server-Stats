...
if(extension_loaded('gd') && function_exists('gd_info'))
{
	...
	if(!empty($sid) && in_array($sid,$ServerIDs))
	{
		...
		if(@mysqli_num_rows($Basic_q) != 0)
		{
			...
			// bf3 logo
			$logo = imagecreatefrompng('./images/bf3.png');
			...
			else
			{
				...
				if($location == '')
				{
					...
					{
						// query for server info
						$Location_q = @mysqli_query($BF3stats,"
							SELECT `CountryCode`
							FROM `tbl_playerdata`
			...
			{
				$width = mysqli_real_escape_string($BF3stats, strip_tags($_GET['w']));
			}
			else
			{
				$width = 560;
			...
			{
				$height = mysqli_real_escape_string($BF3stats, strip_tags($_GET['h']));
			}
			else
			{
				$height = 95;
