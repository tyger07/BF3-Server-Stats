...
else
{
	...
	$numrows = cache_total_players($ServerID, $valid_ids, $GameID, $BF3stats);
	echo '</div>';
}
...
{
	echo '<div style="position: relative;">';
	$Players_q = cache_top_twenty($ServerID, $valid_ids, $GameID, $BF3stats);
	echo '</div>';
}
...
if(@mysqli_num_rows($Players_q) != 0)
{
	...
	while($Players_r = @mysqli_fetch_assoc($Players_q))
	{
		...
		$Soldier_Name = mysqli_real_escape_string($BF3stats, $Players_r['SoldierName']);
		$PlayerID = $Players_r['PlayerID'];
		$Player_ID = mysqli_real_escape_string($BF3stats, $Players_r['PlayerID']);
