...
if(@mysqli_num_rows($Basic_q) != 0)
{
	...
	$battlelog = 'http://battlelog.battlefield.com/bf3/servers/pc/?filtered=1&amp;expand=0&amp;useAdvanced=1&amp;q=' . urlencode($name);
...
$Scoreboard_q = @mysqli_query($BF3stats,"
	SELECT `TeamID`
	...
	while($Scoreboard_r = @mysqli_fetch_assoc($Scoreboard_q))
	{
		...
		if($this_team != $last_team)
		{
			...
			if(@mysqli_num_rows($Team_q) != 0)
			{
				...
				{
					$player = mysqli_real_escape_string($BF3stats, $Team_r['Soldiername']);
