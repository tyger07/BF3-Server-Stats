...
function session_count($userip, $ServerID, $valid_ids, $GameID, $BF3stats, $page, $pid, $player, $isbot)
{
	...
	if(@mysqli_num_rows($exist_query) != 0)
	{
		...
		elseif($now_timestamp - $previous_session_time <= 1)
		{
			...
			die ("<title>BF3 Player Stats - Denied</title></head><body><br/><br/><br/><br/><center><b>You are exceeding the number of requests allowed within a short period of time. <a href='" . $redirect . "'>Try again.</a></b></center></body></html>");
		}
		if(!empty($ServerID))
		{
			@mysqli_query($BF3stats,"
