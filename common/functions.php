...
function Statsout($damagetype,$weapon_array,$PlayerID,$ServerID,$valid_ids,$GameID,$BF3stats,$ID)
{
	...
	if(!empty($ServerID) && ($damagetype != 'VehicleCustom'))
	{
		...
		$Weapon_q = @mysqli_query($BF3stats,"
	...
	elseif($damagetype == 'VehicleCustom')
	{
		...
		{
			// see if this player has used this category's weapons
			$Vehicle_q = @mysqli_query($BF3stats,"
...
function rank($ServerID,$valid_ids,$PlayerID,$BF3stats,$GameID)
{
	...
	if(!empty($ServerID))
	{
		$KillsC_q = @mysqli_query($BF3stats,"
			SELECT `rank`, `timestamp`
			FROM `tyger_stats_rank_cache`
			WHERE `PlayerID` = {$PlayerID}
			AND `category` = 'Kills'
	...
	else
	{
		...
		if(@mysqli_num_rows($TotalRowsC_q) != 0)
		{
			...
			if(($timestamp <= $old) OR ($Players == 0))
			{
				...
				$TotalRows_q = @mysqli_query($BF3stats,"
...
function session_count($userip, $ServerID, $valid_ids, $GameID, $BF3stats, $page, $pid, $player, $isbot)
{
	...
	if(!empty($ServerID))
	{
		$exist_query = @mysqli_query($BF3stats,"
	...
	if(@mysqli_num_rows($exist_query) != 0)
	{
		...
		elseif($now_timestamp - $previous_session_time <= 1)
		{
			...
			die ("<title>BF3 Player Stats - Denied</title></head><meta http-equiv='refresh' content='2' /><body><div id='pagebody'><div class='subsection'><div class='headline'>You are exceeding the number of requests allowed within a short period of time. <a href='" . $redirect . "'>Try again.</a></div></div></div></body></html>");
		}
		if(!empty($ServerID))
		{
			@mysqli_query($BF3stats,"
	...
	$old_query = @mysqli_query($BF3stats,"
		SELECT `IP`
		FROM `tyger_stats_sessions`
		WHERE `timestamp` <= '{$old}'
	");
	...
	if(!empty($ServerID))
	{
		$ses_count = @mysqli_query($BF3stats,"
...
function cache_total_players($ServerID, $valid_ids, $GameID, $BF3stats)
{
	// check to see if this count cache table exists
	@mysqli_query($BF3stats,"
		CREATE TABLE IF NOT EXISTS `tyger_stats_count_cache`
...
function cache_total_suspects($ServerID, $valid_ids, $GameID, $BF3stats)
{
	// check to see if this count cache table exists
	@mysqli_query($BF3stats,"
		CREATE TABLE IF NOT EXISTS `tyger_stats_count_cache`
...
function cache_total_chat($ServerID, $valid_ids, $GameID, $BF3stats)
{
	// check to see if this count cache table exists
	@mysqli_query($BF3stats,"
		CREATE TABLE IF NOT EXISTS `tyger_stats_count_cache`
...
function cache_top_twenty($ServerID, $valid_ids, $GameID, $BF3stats)
{
	...
	else
	{
		...
		if(!empty($ServerID))
		{
			$Players_q  = @mysqli_query($BF3stats,"
		...
		while($Players_r = @mysqli_fetch_assoc($Players_q))
		{
			...
			$SoldierName = mysqli_real_escape_string($BF3stats, $Players_r['SoldierName']);
