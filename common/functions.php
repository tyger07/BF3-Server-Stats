...
function session_count($userip, $ServerID, $valid_ids, $GameID, $BF3stats, $page, $pid, $player, $isbot)
{
	...
	if(@mysqli_num_rows($exist_query) != 0)
	{
		...
		elseif($now_timestamp - $previous_session_time <= 1)
		{
			...
			die ("<title>BF3 Player Stats - Denied</title></head><body><div id='pagebody'><div class='subsection'><div class='headline'>You are exceeding the number of requests allowed within a short period of time. <a href='" . $redirect . "'>Try again.</a></div></div></div></body></html>");
		}
		if(!empty($ServerID))
		{
			@mysqli_query($BF3stats,"
